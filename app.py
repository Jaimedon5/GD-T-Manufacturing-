...
gdt_db = {
    ...
    'Perfil de una línea': {'sym': '⌒', 'type': 'profile', 'datum': False, 'def': 'Forma de una curva 2D.', 'app': 'Alas, levas.', 'why': 'Aerodinámica.', 'geo': 'curved_surf'},
    'Perfil de una superficie': {'sym': '⌓', 'type': 'profile', 'datum': False, 'def': 'Forma de superficie 3D.', 'app': 'Moldes, carrocería.', 'why': 'Estética.', 'geo': 'curved_surf'},
...
# ==========================================
# VISTA 1: SIMULACIÓN 3D (GEOMETRÍAS REALES)
# ==========================================
def plot_3d(feature, tol):
    ...
    if feature == 'Rectitud':
        ...
        fig.add_trace(go.Scatter3d(x=x_real, y=np.zeros_like(z), z=z, mode='lines', line=dict(color='blue', width=12), name='Eje Real (Desviado)'))
        ...
        fig.add_trace(go.Scatter3d(x=np.zeros_like(z), y=np.zeros_like(z), z=z, mode='lines', line=dict(color='black', dash='dash', width=5), name='Nominal'))

    # 2. PLANICIDAD (SUPERFICIE)
        ...
        fig.add_trace(go.Surface(z=np.full_like(xg, tol), x=xg, y=yg, opacity=0.2, colorscale='Reds', showscale=False, name='Lim Sup'))
        fig.add_trace(go.Surface(z=np.full_like(xg, -tol), x=xg, y=yg, opacity=0.2, colorscale='Reds', showscale=False, name='Lim Inf'))

    # 3. POSICIÓN (PLACA CON AGUJERO)
    elif feature == 'Posición':
        x = np.linspace(-5,5,20); y=np.linspace(-5,5,20); xg,yg=np.meshgrid(x,y)
        fig.add_trace(go.Surface(z=np.zeros_like(xg), x=xg, y=yg, opacity=0.2, colorscale='Greys', showscale=False, name='Placa'))
        fig.add_trace(go.Scatter3d(x=[1,1], y=[1,1], z=[-2,5], line=dict(color='red', width=10), name='Eje Real'))
        fig.add_trace(go.Scatter3d(x=[0,0], y=[0,0], z=[-2,5], line=dict(color='black', dash='dash', width=5), name='Teórico'))
    ...
    # 4. PERPENDICULARIDAD (ESCUADRA)
        ...
        fig.add_trace(go.Surface(x=X, y=Y, z=np.full_like(Y, tol), opacity=0.2, showscale=False, colorscale='Reds'))
        fig.add_trace(go.Surface(x=X, y=Y, z=np.full_like(Y, -tol), opacity=0.2, showscale=False, colorscale='Reds'))
    ...
    # 6. CILINDRICIDAD / REDONDEZ (DEFECTO)
    else:
        r = 5 + 0.2*np.sin(zg)
...
# ==========================================
# VISTA 2: MONTAJE REAL (ANIMACIONES ESTABLES)
# ==========================================
def plot_real_anim(feature):
    ...
    # CASO 1: ROTACIÓN (PIEZA GIRA, RELOJ FIJO)
    if feature in ['Redondez', 'Cilindricidad', 'Alabeo Circular', 'Alabeo Total', 'Concentricidad']:
        ...
        draw_rect(fig, 1, 3, 8, 2, color="blue")
        ...
        # Animación de la aguja (simulando lectura)
        for i in range(50):
            dy = 0.3 * np.sin(i*0.5)
            frames.append(go.Frame(data=[
                go.Scatter(x=[5, 5], y=[6, 7.5+dy], mode='lines', line=dict(color='red', width=3))
            ]))
        # Estado inicial aguja
        fig.add_trace(go.Scatter(x=[5, 5], y=[6, 7.5], mode='lines', line=dict(color='red', width=3), name='Aguja'))

    # CASO 2: ANGULARIDAD (MESA DE SENOS)
    elif feature == 'Angularidad':
        draw_rect(fig, 0, 0, 10, 0.5, fill="#aaa") # Granito
        ...
        draw_rect(fig, 2, 3.5, 6, 2, color="black") # Pieza nivelada
        
        # Reloj se mueve
        for i in range(50):
            x = 2 + i/10
            frames.append(go.Frame(data=[
                go.Scatter(x=[x, x], y=[5.5, 7], mode='lines+markers', line=dict(color='red'))
            ]))
        fig.add_trace(go.Scatter(x=[2, 2], y=[5.5, 7], mode='lines+markers', line=dict(color='red'), name='Reloj'))

    # CASO 3: DESLIZAMIENTO (RECTITUD/PLANICIDAD/PERFIL)
    else:
        ...
        fig.add_trace(go.Scatter(x=[1, 1], y=[2.2, 4.2], mode='lines+markers', line=dict(color='red'), name='Reloj'))

    fig.frames = frames
    return fig

# ==========================================
# VISTA 3: PLANO TÉCNICO (DIBUJO TÉCNICO ASME)
# ==========================================
def draw_blueprint(feature, tol_val):
    ...
    # --- DIBUJO DE LA PIEZA ---
    if ftype == 'axis': # EJE (Cilindricidad, Posición, Redondez)
        ...
        # LA FLECHA TOCA LA COTA DE TAMAÑO (Regla ASME para Ejes)
        leader_target = (10.5, 4.5)
            
    else: # SUPERFICIE (Rectitud, Perfil, Planicidad)
        draw_rect(fig, 3, 2, 6, 3, width=3)
        # LA FLECHA TOCA LA SUPERFICIE (Regla ASME para Superficies)
        leader_target = (6, 5)
    ...
    if datum:
        ...
        # Triángulo de Datum en la pieza
        if ftype == 'surf':
            fig.add_annotation(x=6, y=2, text="<b>A</b>", showarrow=True, arrowhead=2, ay=20)
        else:
            fig.add_annotation(x=2, y=5, text="<b>A</b>", showarrow=True, arrowhead=2, ay=0, ax=-20)
...
def draw_master(active_features):
    ...
    for feat in active_features:
        if feat in locs:
            ...
            txt = f"{info['symbol']} 0.1 {info.get('datum','')}"
...
if mode == "🔬 Análisis Individual":
    ...
    # Clave Única (Evita que se trabe)
    ukey = f"{feat}_{view}_{tol}_{uuid.uuid4()}"
    info = gdt_db.get(feat, gdt_db['Rectitud'])

    # Tarjeta de Información (SIEMPRE VISIBLE ARRIBA)
    st.markdown(f"""
    <div class="info-card">
    ...
    st.sidebar.success("Constructor Activo")
    sel = st.sidebar.multiselect("Agregar cotas:", list(gdt_db.keys()), default=['Rectitud', 'Posición'])
    st.plotly_chart(draw_master(sel), use_container_width=True, key=f"master_{uuid.uuid4()}")
